...
export default function TeacherApplications() {
    ...
    getApplicationsForProject(id)
      .then((res) => {
        ...
        const readyForReview = res.data.applications.filter(
            app => app.status === 'pending_faculty_approval' || app.status === 'approved' || app.status === 'rejected'
        );
  ...
  const handleAccept = (applicationId) => {
    ...
    // TODO: API call to update application status to 'approved'
  };

  const handleReject = (applicationId) => {
    alert(`Rejected application: ${applicationId}`);
    // TODO: API call to update application status to 'rejected'
  };
      ...
      <div className="relative max-w-5xl mx-auto z-10">
        <header className="flex items-center justify-between mb-10">
          <div className='flex items-center gap-4'>
            <button onClick={() => navigate('/teacher-dashboard')} className="p-2 rounded-full bg-slate-800/50 hover:bg-slate-700/70 transition">
